This is a complete, copy-pasteable Swift API Manager (`GeminiManager.swift`) designed for the latest Gemini Developer API.

It includes support for **Gemini 3 / 2.5 (Text, Vision, Video Analysis)**, **Thinking Mode**, **Imagen 3 (Image Gen)**, and **Veo (Video Gen)**.

### Instructions for the Agent (or Developer)

1.  **Add to Project:** Drop this file (`GeminiManager.swift`) into your Xcode project.
2.  **Get API Key:** Get a key from [Google AI Studio](https://aistudio.google.com/).
3.  **Usage:** Initialize with `let gemini = GeminiManager(apiKey: "YOUR_KEY")`.
4.  **Important:**
      * **Thinking Mode:** Use `thinkingLevel: .high` in `generateContent` for complex logic.
      * **Files:** For video/image analysis, convert your media to `Data` and pass it to `generateContent`.
      * **Context:** This manager handles the "Thought Signature" automatically for Gemini 3, ensuring reasoning is preserved in chat history.
