...
final class ModelRegistry: ObservableObject {
    ...
    func fetchAllModels(forceRefresh: Bool = false) async {
        ...
        logger.info(
            "Model fetch complete. Providers loaded: \(self.modelsByProvider.keys.joined(separator: ", "))"
        )
    }
    ...
    func fetchModelsForProvider(_ provider: KeychainStore.ProviderKey, forceRefresh: Bool = false)
        ...
        if !forceRefresh, let cached = cachedEntry {
            ...
            if age < cacheExpiration {
                ...
                return overlayWithProvidersConfig(
                    providerID: canonicalProviderID, fetchedModels: cached.models)
            }
        }
        ...
        if canonicalProviderID == "anthropic" || canonicalProviderID == "xai" {
            ...
            if hasAPIKey {
                do {
                    ...
                    if fetched.isEmpty {
                        logger.warning(
                            "Official /models fetch returned empty for \(canonicalProviderID); will fall back"
                        )
                    } else {
                ...
                logger.info(
                    "No API key for \(canonicalProviderID); skipping official /models fetch")
            }
            ...
            // 4) curated
            logger.info(
                "📚 Using curated fallback for \(canonicalProviderID): \(curatedDefaults.count) models"
            )
        ...
        do {
            ...
            if !modelsDevModels.isEmpty {
                ...
                return overlayWithProvidersConfig(
                    providerID: canonicalProviderID, fetchedModels: models)
            } else {
        ...
        if hasAPIKey {
            do {
                ...
                logger.warning(
                    "Provider API fetch failed for \(providerID): \(error.localizedDescription)")
            }
        } else {
    ...
    private func shouldAdoptDisplayName(
        _ incoming: String, forModelID id: String, currentDisplayName: String
    ) -> Bool {
    ...
    private func loadCachedModels() {
        guard let data = UserDefaults.standard.data(forKey: "ModelRegistryCache"),
            let cache = try? JSONDecoder().decode([String: CachedModels].self, from: data)
        else {
    ...
    private func saveCachedModels() {
        let cache = modelCache.mapValues {
            CachedModels(models: $0.models, fetchedAt: $0.fetchedAt)
        }
