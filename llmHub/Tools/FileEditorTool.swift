...
final class FileEditorTool: Tool, @unchecked Sendable {
        ...
        [
            ...
            "properties": [
                "operation": [
                    ...
                    "description":
                        "The operation to perform: create, edit, append, delete, rename, move, or copy",
                ],
                "path": [
                    "type": "string",
    ...
    nonisolated func execute(input: [String: Any]) async throws -> String {
        guard let operationStr = input["operation"] as? String,
            let operation = FileOperation(rawValue: operationStr)
        else {
    ...
    private func validateRequest(_ request: FileOperationRequest) throws {
        ...
        case .rename, .move, .copy:
            ...
            guard request.destination != nil else {
                throw FileOperationError.invalidOperation(
                    "\(request.operation.rawValue) requires destination")
            }
        }
    }
    ...
    private func generatePreview(for request: FileOperationRequest) throws -> FileOperationPreview {
        ...
        case .edit:
            if fm.fileExists(atPath: url.path) {
                ...
                if let oldStr = request.oldString, let newStr = request.newString {
                    proposedContent = originalContent?.replacingOccurrences(
                        of: oldStr, with: newStr)
                    diffLines = generateDiff(
                        original: originalContent ?? "", proposed: proposedContent ?? "")
                }
            }
        ...
        case .append:
            if fm.fileExists(atPath: url.path) {
                ...
                diffLines = appendedLines.enumerated()
                    .map {
                        DiffLine(
                            type: .added, content: $0.element, lineNumber: startLine + $0.offset + 1
                        )
                    }
            }
        ...
        case .delete:
            if fm.fileExists(atPath: url.path) {
                ...
                diffLines = (originalContent ?? "").components(separatedBy: .newlines)
                    .enumerated()
                    .map {
                        DiffLine(type: .removed, content: $0.element, lineNumber: $0.offset + 1)
                    }
            }
        ...
        case .rename, .move, .copy:
            ...
            diffLines = [
                DiffLine(
                    type: .unchanged, content: "\(actionVerb): \(request.path)", lineNumber: nil),
                DiffLine(
                    type: .unchanged, content: "To: \(request.destination ?? "?")", lineNumber: nil),
            ]
        }
    ...
    private func generateDiff(original: String, proposed: String) -> [DiffLine] {
        ...
        while i < originalLines.count || j < proposedLines.count {
            if i >= originalLines.count {
                ...
                diffLines.append(
                    DiffLine(type: .added, content: proposedLines[j], lineNumber: j + 1))
                j += 1
            } else if j >= proposedLines.count {
                // Remaining lines are deletions
                diffLines.append(
                    DiffLine(type: .removed, content: originalLines[i], lineNumber: i + 1))
                i += 1
            } else if originalLines[i] == proposedLines[j] {
                // Lines match
                diffLines.append(
                    DiffLine(type: .unchanged, content: originalLines[i], lineNumber: i + 1))
                i += 1
                j += 1
            } else {
