...
final class CodeInterpreterTool: Tool, @unchecked Sendable {
    ...
    func executeWithResult(code: String, language: SupportedLanguage) async throws -> CodeExecutionResult {
        // Check for approval if required
        if securityMode == .approval {
            guard let handler = approvalHandler else {
                throw CodeExecutionError.approvalDenied
...
extension CodeInterpreterTool {
    ...
    func validateCode(_ code: String, language: SupportedLanguage) -> [String] {
        ...
        switch language {
        case .swift:
            dangerousPatterns.append(("Process\\(", "⚠️ Code uses Process which can spawn subprocesses"))
            dangerousPatterns.append(("FileManager", "⚠️ Code uses FileManager (file system access)"))
        case .python:
            dangerousPatterns.append(("import os", "⚠️ Code imports 'os' module (file system access)"))
            dangerousPatterns.append(("import subprocess", "⚠️ Code imports 'subprocess' (shell access)"))
        case .javascript, .typescript:
            dangerousPatterns.append(("require\\(['\"]child_process", "⚠️ Code uses child_process (shell access)"))
            dangerousPatterns.append(("require\\(['\"]fs", "⚠️ Code uses fs module (file system access)"))
        case .dart:
            dangerousPatterns.append(("import 'dart:io'", "⚠️ Code imports dart:io (file system/process access)"))
        }
